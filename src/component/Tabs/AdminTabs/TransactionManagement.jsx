...
const TokenTransactionManagement = () => {
    ...
    const fetchTransactions = async (page = 1, limit = transactionsPerPage) => {
        ...
        try {
            ...
            setTimeout(async () => {
                try {
                    const response = await api.get(`/api/transaction/token`, {
                        params: {
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-xl font-semibold">{t("admin:token.transaction.title")}</h1>
            </div>
                        ...
                        <table className="table-auto w-full border-collapse border border-gray-200 dark:text-gray-300/80 dark:border-gray-900 dark:bg-gray-900">
                            <thead>
                                <tr className="bg-gray-100 dark:bg-gray-600 dark:text-gray-100/80">
                                    ...
                                    <th className="border border-gray-200 px-4 py-2">{t("admin:token.transaction.table.sender")}</th>
                                    <th className="border border-gray-200 px-4 py-2">{t("admin:token.transaction.table.receiver")}</th>
                                    <th className="border border-gray-200 px-4 py-2">{t("common:table.amount")}</th>
                                    <th className="border border-gray-200 px-4 py-2">{t("admin:token.transaction.table.date")}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {transactions.length > 0 ? (
                                    ...
                                    <tr>
                                        ...
                                        {t("admin:token.transaction.not_found_message")}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
